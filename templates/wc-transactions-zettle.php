	...
	<table class="woocommerce-transactions-table woocommerce-MyAccount-transactions shop_table shop_table_responsive my_account_transactions account-transactions-table">
		...
		<tbody>
			...
			foreach ( $transactions as $transaction ) {
				...
				$grand_total = usbswiper_get_zettle_transaction_total($transaction->ID);
	...
	<div class="woocommerce-pagination woocommerce-pagination--without-numbers woocommerce-Pagination">
		<?php if ( 1 !== $current_page ) :
            ...
            if( is_wc_endpoint_url('zettle-transactions') ){
                $next_page_url = add_query_arg($next_args,  wc_get_endpoint_url( 'zettle-transactions' ,'') );
            }
		...
		<?php if ( intval( $max_num_pages ) !== $current_page ) :
            ...
            $previous_page_url = '';
            if( is_wc_endpoint_url('zettle-transactions') ){
                $previous_page_url = add_query_arg($previous_args,  wc_get_endpoint_url( 'zettle-transactions' ,'') );
            }
            ?>
